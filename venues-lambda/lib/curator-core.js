...
const ENTITY_TABLES = Object.freeze({ artist: 'bndy-artists', venue: 'bndy-venues', event: 'bndy-events', festival: 'bndy-festivals' });
let JWT_SECRET = null;
...
function normalisePostcodeToken(value) { return typeof value === 'string' ? value.toUpperCase().replace(/\s+/g, '') : ''; }
function normaliseCuratorAccess(dbUser) {
  ...
  const prefixes = Array.isArray(raw.postcode_prefixes) ? raw.postcode_prefixes.map(normalisePostcodeToken).filter(Boolean) : [];
  return { scope: raw.scope === 'postcode' ? 'postcode' : 'global', postcodePrefixes: [...new Set(prefixes)], ownRecordsOnly: raw.own_records_only === true };
}
...
function postcodeAllowed(access, postcode) {
  ...
  return access.postcodePrefixes.some((token) => /^[A-Z]{1,2}$/.test(token) ? parts.area === token : parts.outward === token);
}
function creatorOf(item) { return item?.createdBy || item?.created_by || item?.created_by_user_id || null; }
...
async function assertEntityAllowed(dynamodb, gate, access, entityType, item, venueCache, proposedVenueIds) {
  ...
  if (access.ownRecordsOnly && !mine) return { error: 'This curator can only modify records they created', statusCode: 403 };
  if (access.scope !== 'postcode') return null;
  if (entityType === 'artist') return mine ? null : { error: 'Postcode-restricted curators can only modify artists they created', statusCode: 403 };
  if (entityType === 'venue') return postcodeAllowed(access, item.postcode || item.postalCode || item.postal_code) ? null : { error: 'Venue is outside this curator’s postcode access', statusCode: 403 };
  if (entityType === 'event') {
    const postcode = await getVenuePostcode(dynamodb, item.venueId || item.venue_id, venueCache);
    return postcodeAllowed(access, postcode) ? null : { error: 'Event venue is outside this curator’s postcode access', statusCode: 403 };
  }
  if (entityType === 'festival') {
    ...
    if (venueIds.length === 0) return mine ? null : { error: 'Festival has no venue postcode to match this curator’s access', statusCode: 403 };
    for (const venueId of venueIds) {
      const postcode = await getVenuePostcode(dynamodb, venueId, venueCache);
      if (!postcodeAllowed(access, postcode)) return { error: 'Festival includes a venue outside this curator’s postcode access', statusCode: 403 };
    }
  }
  return null;
}
...
async function enforceCuratorPolicy(deps, event, gate) {
  ...
  if (path.endsWith('/festival-tag')) {
    ...
    const ids = [...new Set([...(Array.isArray(body.add) ? body.add : []), ...(Array.isArray(body.remove) ? body.remove : [])].filter(Boolean))];
    for (const id of ids) {
...
async function requireRole(deps, event, roles) {
  ...
  let session;
  try { const secret = await getJWTSecret(ssm); session = jwt.verify(sessionToken, secret); }
  catch { return { error: 'Invalid session', statusCode: 401 }; }
  const userResult = await dynamodb.get({ TableName: USERS_TABLE, Key: { cognito_id: session.userId } }).promise();
  ...
  if (!(dbUser.platformAdmin || roles.includes(role))) return { error: 'Curator access required', statusCode: 403 };
...
async function logActivity(dynamodb, { actorCognitoId, actorName, action, entityType, entityId, entityName, detail }) {
  try {
    ...
    await dynamodb.put({ TableName: ACTIVITY_TABLE, Item: { user_id: actorCognitoId, sk: `${at}#${suffix}`, at, actor_name: actorName || null, action, entity_type: entityType, entity_id: entityId, entity_name: entityName || null, detail: detail || null, gsi_pk: 'ALL' } }).promise();
  } catch (error) { console.error('[CURATOR] Activity log write failed:', error.message); }
}
function pickFields(body, allowed) { const out = {}; for (const key of allowed) if (body[key] !== undefined) out[key] = body[key]; return out; }
async function hideEntity(dynamodb, { tableName, id, actor, reason, extraSet = {} }) {
  ...
  Object.entries(extraSet).forEach(([k, v], i) => { names[`#x${i}`] = k; values[`:x${i}`] = v; expr += `, #x${i} = :x${i}`; });
  await dynamodb.update({ TableName: tableName, Key: { id }, ConditionExpression: 'attribute_exists(id)', UpdateExpression: expr, ExpressionAttributeNames: names, ExpressionAttributeValues: values }).promise();
}
async function restoreEntity(dynamodb, { tableName, id, extraSet = {} }) {
  const names = { '#hidden': 'hidden' };
  const values = { ':false': false };
  const setParts = ['#hidden = :false'];
  Object.entries(extraSet).forEach(([k, v], i) => { names[`#x${i}`] = k; values[`:x${i}`] = v; setParts.push(`#x${i} = :x${i}`); });
  await dynamodb.update({ TableName: tableName, Key: { id }, ConditionExpression: 'attribute_exists(id)', UpdateExpression: `SET ${setParts.join(', ')} REMOVE hidden_by, hidden_at, hidden_reason`, ExpressionAttributeNames: names, ExpressionAttributeValues: values }).promise();
}
module.exports = { requireRole, logActivity, pickFields, hideEntity, restoreEntity, normaliseCuratorAccess, postcodeAllowed, postcodeParts };
