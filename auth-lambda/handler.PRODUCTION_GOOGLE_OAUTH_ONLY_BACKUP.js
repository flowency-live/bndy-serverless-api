...
const handleOAuthCallback = async (event) => {
  ...
  try {
    ...
    const picture = decodedIdToken.picture || null;
  ...
  try {
    ...
    const responseData = {
      user: {
        ...
        oauthProfilePicture: dbUser.oauth_profile_picture || null,
...
const createOrUpdateUser = async (userData) => {
  const { cognitoId, email, username, oauthProfilePicture } = userData;
  ...
  try {
    ...
    if (existingUser.Item) {
      ...
      // Update existing user (OAuth picture can change)
      await dynamodb.update({
        TableName: USERS_TABLE,
        Key: { cognito_id: cognitoId },
        UpdateExpression: 'SET email = :email, username = :username, oauth_profile_picture = :oauthPicture, updated_at = :updatedAt',
        ExpressionAttributeValues: {
          ':email': email,
          ':username': username,
          ':oauthPicture': oauthProfilePicture,
      ...
      await dynamodb.put({
        ...
        Item: {
          ...
          display_name: null,
          avatar_url: null,
          oauth_profile_picture: oauthProfilePicture,
          instrument: null,
