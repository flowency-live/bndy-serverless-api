...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Integration API endpoint (API key required)
    if (method === 'POST' && path === '/api/integration/artists') {
      return await handleIntegrationFindOrCreateArtist(event);
    }
...
async function handleGetArtistByExternalId(event) {
  ...
  try {
    ...
    const artist = {
      ...
      artist_type: matchingArtist.artist_type || null,
      artistType: matchingArtist.artist_type || null,
      bio: matchingArtist.bio || '',
      location: matchingArtist.location || '',
      locationLat: matchingArtist.locationLat || null,
      locationLng: matchingArtist.locationLng || null,
      ...
      source: matchingArtist.source || null,
      ai_created: matchingArtist.ai_created || false,
...
async function handleSearchArtists(event) {
  ...
  try {
    ...
    const result = await dynamodb.query({
      ...
      ProjectionExpression: 'id, #name, #location, locationLat, locationLng, locationType, profileImageUrl, external_ids',
      ExpressionAttributeNames: {
        '#name': 'name',
        '#location': 'location'
      },
...
/**
 * Integration API: Find or Create Artist
 *
 * Searches for existing artist by name (using GSI), returns if found with high match score.
 * Creates new artist if no match found.
 *
 * Request: { name, location, facebookUrl?, instagramUrl?, websiteUrl?, spotifyUrl? }
 * Response: { success, artist, isNew, matchMethod }
 */
async function handleIntegrationFindOrCreateArtist(event) {
  console.log('[INTEGRATION] Artists Lambda: Find-or-create artist');
  ...
  const { name, location, facebookUrl, instagramUrl, websiteUrl, spotifyUrl, bio, genres } = body;
  ...
  try {
    // 3. Search for existing artist using GSI (reuse existing search logic)
    const searchTerm = name.toLowerCase().trim();
    const prefix = searchTerm.substring(0, 2);

    const searchResult = await dynamodb.query({
      TableName: 'bndy-artists',
      IndexName: 'name-search-index',
      KeyConditionExpression: 'name_prefix = :prefix AND begins_with(name_lower, :searchTerm)',
      ExpressionAttributeValues: {
      ...
      ProjectionExpression: 'id, #name, #location, locationLat, locationLng, locationType, profileImageUrl, facebookUrl, instagramUrl, websiteUrl, spotifyUrl',
      ExpressionAttributeNames: {
        '#name': 'name',
        '#location': 'location'
      },
    ...
    const matches = (searchResult.Items || [])
      ...
      .filter(artist => artist.matchScore >= 80)  // Only consider high-confidence matches
      .sort((a, b) => b.matchScore - a.matchScore);

    // 5. If high-confidence match found, return existing artist
    if (matches.length > 0) {
      const existingArtist = matches[0];
      console.log(`[INTEGRATION] Found existing artist: ${existingArtist.id} (${existingArtist.name}) with score ${existingArtist.matchScore}`);
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          artist: {
            ...
            locationLat: existingArtist.locationLat || null,
            locationLng: existingArtist.locationLng || null,
            locationType: existingArtist.locationType || null,
            profileImageUrl: existingArtist.profileImageUrl || null,
            facebookUrl: existingArtist.facebookUrl || null,
            instagramUrl: existingArtist.instagramUrl || null,
            websiteUrl: existingArtist.websiteUrl || null,
            spotifyUrl: existingArtist.spotifyUrl || null,
            matchConfidence: existingArtist.matchScore
          },
          isNew: false,
    ...
    // 6. No match found - create new artist
    console.log(`[INTEGRATION] No match found for "${name}" - creating new artist`);
    ...
    const newArtist = {
      id: artistId,
      name: name.trim(),
      ...generateNameSearchFields(name.trim()),
      location: location.trim(),
      locationLat: null,
      locationLng: null,
      locationType: null,
    ...
    console.log(`[INTEGRATION] Created new artist: ${artistId} (${name})`);
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        artist: {
          ...
          facebookUrl: newArtist.facebookUrl || null,
          instagramUrl: newArtist.instagramUrl || null,
          websiteUrl: newArtist.websiteUrl || null,
          spotifyUrl: newArtist.spotifyUrl || null,
          matchConfidence: 0
        },
        isNew: true,
        matchMethod: 'new_artist_created'
      })
    };

  } catch (error) {
    console.error('[INTEGRATION] Artist find-or-create failed:', error);
    return {
      statusCode: 500,
      headers: getIntegrationHeaders(),
      body: JSON.stringify({ error: 'Internal server error' })
